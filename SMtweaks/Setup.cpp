...
void init_secret_sharing()
{
  ...
  ifstream shamirEvalPoints("Data/evalPoints.txt");
  string str;
  input >> str;
  unsigned int n;
  input >> n;
  ...
  unsigned int evalPoints [n];
  for (unsigned int i = 0; i < n; i++){
    shamirEvalPoints >> evalPoints[i];
  }
  shamirEvalPoints.close(); 
  ...
  switch (v)
    {
      ...
      case 2:
        ...
        SD.Initialize_Shamir(n, t, evalPoints);
        break;
      case 3:
